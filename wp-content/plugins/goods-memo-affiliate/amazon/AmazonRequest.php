...
class AmazonRequest {
	...
	public static function requestSearchIndex(URLInfo $urlInfo, CommonRESTParameter $commonParameter, RESTParameter $restParameter, int $itemCount, $retryCount = 3) {
		...
		for($i = 0; $i < $retryCount; $i ++) { // リトライ回数：3回 //TODO 設定画面で指定する
			if ($i >= 1) {
				sleep ( 1 ); // 再試行の待ち時間（1秒）
			}
