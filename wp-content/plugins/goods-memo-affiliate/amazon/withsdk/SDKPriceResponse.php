...
class SDKPriceResponse {
	...
	public static function makePriceItem(\Amazon\ProductAdvertisingAPI\v1\com\amazon\paapi5\v1\Item $searchItem, float $priceTime): PriceItem {
		...
		$priceFlag = 0b000;
		if ($offers != NULL and $offers->getListings () != NULL and $offers->getListings () [0] != NULL and $offers->getListings () [0]->getPrice () != NULL and $offers->getListings () [0]->getPrice ()->getAmount () != NULL) {
			$priceFlag = $priceFlag | 0b001; // 「Amazon.co.jp 価格」が存在する。
		}
		if ($offers != NULL and $offers->getSummaries () != NULL and $offers->getSummaries () [0] != NULL and $offers->getSummaries () [0]->getLowestPrice () != NULL and $offers->getSummaries () [0]->getLowestPrice ()->getAmount () != NULL) {
		...
		if ($offers != NULL and $offers->getListings () != NULL and $offers->getListings () [0] != NULL and $offers->getListings () [0]->getDeliveryInfo () != NULL) {
