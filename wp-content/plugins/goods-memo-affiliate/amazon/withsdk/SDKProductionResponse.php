...
/**
 * Description of SDKProductionResponse
 *
 * @author Goods Memo
 */
class SDKProductionResponse {
	...
	public static function makeProductionItem(\Amazon\ProductAdvertisingAPI\v1\com\amazon\paapi5\v1\Item $searchItem): ProductionItem {
		...
		if ($itemInfo->getClassifications () != NULL and $itemInfo->getClassifications ()->getBinding () != NULL and $itemInfo->getClassifications ()->getBinding ()->getDisplayValue () != NULL) {
		...
		if ($itemInfo->getByLineInfo () != NULL) {
			...
			if ($byLineInfo->getManufacturer () != NULL and $byLineInfo->getManufacturer ()->getDisplayValue () != NULL) {
				...
				if ($manufacturer->getLabel () != NULL and empty ( $productionItem->getManufacturerLabel () )) {
					...
					$manufacturerLabel = str_replace ( SDKProductionResponse::MANUFACTURER_LABEL_SEARCH, SDKProductionResponse::MANUFACTURER_LABEL_REPLACE, $manufacturerLabel );
